...
Eventful.prototype.log = function(options, args){
  ...
  for(level in options){
    if (options[level].exclude && options[level].exclude.indexOf(name) > -1){ continue; }
    if (options[level].only && options[level].only.indexOf(name) === -1){ continue; }
...
Eventful.prototype.format = function(date, name, args, format){
  ...
  text = util.node.inspect(util.node.inspect(args, false, 1));
